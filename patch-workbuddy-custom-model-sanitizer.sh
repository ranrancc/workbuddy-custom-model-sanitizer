...
if (text.includes(exitReplacement)) {
  ...
  const legacyStart = 'if(ev&&this.isCustomModelRequest(el,ec)&&Array.isArray(ev.messages)){';
  const legacyEnd = 'eA&&ev&&"string"==typeof eh&&(eh=JSON.stringify(ev));';
  const start = text.indexOf(legacyStart);
  const end = start >= 0 ? text.indexOf(legacyEnd, start) : -1;
  if (start < 0 || end < 0) {
    console.error("found legacy sanitizer marker, but could not locate legacy sanitizer bounds");
    process.exit(2);
  }
  text = text.slice(0, start) + exitReplacement + text.slice(end + legacyEnd.length);
  changed = true;
