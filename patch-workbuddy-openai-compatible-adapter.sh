...
BACKUP_DIR="${WORKBUDDY_BACKUP_DIR:-$APP_PATH/Contents/Resources/workbuddy-openai-compatible-adapter-backups}"
STAMP="$(date +%Y%m%d-%H%M%S)"

log() {
  printf '[workbuddy-openai-compatible-adapter] %s\n' "$*"
}

fail() {
  printf '[workbuddy-openai-compatible-adapter] ERROR: %s\n' "$*" >&2
  exit 1
}
...
log "Patching outbound OpenAI-compatible request adapter"
...
log "Patching inbound streaming tool-call adapter"
node - "$CLI_BUNDLE" <<'NODE'
const fs = require("fs");
...
const constructorOld =
  'this.textBlockStarted=!1,this.toolBlockStarted=!1,this.thinkingBlockStarted=!1,this.thinkingBlockCompleted=!1,this.thinkingBlockIndex=-1,this.shouldRehydrateBlobRefs=';

const constructorNew =
  'this.textBlockStarted=!1,this.toolBlockStarted=!1,this.thinkingBlockStarted=!1,this.thinkingBlockCompleted=!1,this.thinkingBlockIndex=-1,this.openAICompatibleToolBlocks={},this.shouldRehydrateBlobRefs=';

const responseStartedOld =
  'el?.delta&&(this.textBlockStarted=!1,this.toolBlockStarted=!1,this.thinkingBlockStarted=!1,this.thinkingBlockCompleted=!1,this.thinkingBlockIndex=-1,ea.push({type:"message_start"';

const responseStartedNew =
  'el?.delta&&(this.textBlockStarted=!1,this.toolBlockStarted=!1,this.thinkingBlockStarted=!1,this.thinkingBlockCompleted=!1,this.thinkingBlockIndex=-1,this.openAICompatibleToolBlocks={},ea.push({type:"message_start"';
...
const fallbackToolBlock =
  'if("model"===ei.type&&ei.event?.choices?.[0]){let el=ei.event.choices[0],ec=el.delta?.tool_calls,eu=this.thinkingBlockIndex>=0?this.thinkingBlockIndex+1:0,ed=this.thinkingBlockIndex>=0?this.thinkingBlockIndex+2:1;if(ec&&ec.length>0){let ei=ec[0];ei.id?(this.textBlockStarted&&(ea.push({type:"content_block_stop",index:eu}),this.textBlockStarted=!1),ea.push({type:"content_block_start",index:ed,content_block:{type:"tool_use",id:ei.id,name:ei.function?.name||("string"==typeof ei.id&&ei.id.startsWith("functions.")?ei.id.slice(10).split(":")[0]:""),input:{}}}),this.toolBlockStarted=!0,ea.push({type:"content_block_delta",index:ed,delta:{type:"input_json_delta",partial_json:""}})):ei.function?.arguments!==void 0&&ea.push({type:"content_block_delta",index:ed,delta:{type:"input_json_delta",partial_json:ei.function.arguments}})}el.finish_reason&&("tool_calls"===el.finish_reason?(this.toolBlockStarted&&(ea.push({type:"content_block_stop",index:ed}),this.toolBlockStarted=!1),ei.event.usage&&ea.push({type:"message_delta",delta:{stop_reason:"tool_use",stop_sequence:null},usage:{input_tokens:ei.event.usage.prompt_tokens||0,cache_creation_input_tokens:ei.event.usage.cache_creation_input_tokens||0,cache_read_input_tokens:ei.event.usage.cache_read_input_tokens||0,output_tokens:ei.event.usage.completion_tokens||0}})):("stop"===el.finish_reason||"end_turn"===el.finish_reason)&&this.textBlockStarted&&(ea.push({type:"content_block_stop",index:eu}),this.textBlockStarted=!1))}';

const originalToolBlock =
  'if("model"===ei.type&&ei.event?.choices?.[0]){let el=ei.event.choices[0],ec=el.delta?.tool_calls,eu=this.thinkingBlockIndex>=0?this.thinkingBlockIndex+1:0,ed=this.thinkingBlockIndex>=0?this.thinkingBlockIndex+2:1;if(ec&&ec.length>0){let ei=ec[0];ei.id?(this.textBlockStarted&&(ea.push({type:"content_block_stop",index:eu}),this.textBlockStarted=!1),ea.push({type:"content_block_start",index:ed,content_block:{type:"tool_use",id:ei.id,name:ei.function?.name||"",input:{}}}),this.toolBlockStarted=!0,ea.push({type:"content_block_delta",index:ed,delta:{type:"input_json_delta",partial_json:""}})):ei.function?.arguments!==void 0&&ea.push({type:"content_block_delta",index:ed,delta:{type:"input_json_delta",partial_json:ei.function.arguments}})}el.finish_reason&&("tool_calls"===el.finish_reason?(this.toolBlockStarted&&(ea.push({type:"content_block_stop",index:ed}),this.toolBlockStarted=!1),ei.event.usage&&ea.push({type:"message_delta",delta:{stop_reason:"tool_use",stop_sequence:null},usage:{input_tokens:ei.event.usage.prompt_tokens||0,cache_creation_input_tokens:ei.event.usage.cache_creation_input_tokens||0,cache_read_input_tokens:ei.event.usage.cache_read_input_tokens||0,output_tokens:ei.event.usage.completion_tokens||0}})):("stop"===el.finish_reason||"end_turn"===el.finish_reason)&&this.textBlockStarted&&(ea.push({type:"content_block_stop",index:eu}),this.textBlockStarted=!1))}';

const adapterToolBlock =
  'if("model"===ei.type&&ei.event?.choices?.[0]){let el=ei.event.choices[0],ec=el.delta?.tool_calls,eu=this.thinkingBlockIndex>=0?this.thinkingBlockIndex+1:0,ed=this.thinkingBlockIndex>=0?this.thinkingBlockIndex+2:1;if(ec&&ec.length>0)for(let ei of ec){let eA=Number.isInteger(ei.index)?ei.index:0,eP=ed+eA,eH=this.openAICompatibleToolBlocks[eA]||(this.openAICompatibleToolBlocks[eA]={id:null,name:"",args:"",started:!1});ei.id&&(eH.id=ei.id),ei.function?.name&&(eH.name=ei.function.name);let eG=eH.name||("string"==typeof eH.id&&eH.id.startsWith("functions.")?eH.id.slice(10).split(":")[0]:"");ei.function?.arguments!==void 0&&(eH.args+=ei.function.arguments||"");if(!eH.started&&eG){this.textBlockStarted&&(ea.push({type:"content_block_stop",index:eu}),this.textBlockStarted=!1);let ei=eH.id||`functions.${eG}:${eA}`;ea.push({type:"content_block_start",index:eP,content_block:{type:"tool_use",id:ei,name:eG,input:{}}}),this.toolBlockStarted=!0,eH.started=!0,eH.args&&ea.push({type:"content_block_delta",index:eP,delta:{type:"input_json_delta",partial_json:eH.args}})}else eH.started&&ei.function?.arguments!==void 0&&ea.push({type:"content_block_delta",index:eP,delta:{type:"input_json_delta",partial_json:ei.function.arguments||""}})}el.finish_reason&&("tool_calls"===el.finish_reason?(this.toolBlockStarted&&(Object.keys(this.openAICompatibleToolBlocks).sort((ei,ea)=>Number(ei)-Number(ea)).forEach(ei=>{this.openAICompatibleToolBlocks[ei]?.started&&ea.push({type:"content_block_stop",index:ed+Number(ei)})}),this.toolBlockStarted=!1,this.openAICompatibleToolBlocks={}),ei.event.usage&&ea.push({type:"message_delta",delta:{stop_reason:"tool_use",stop_sequence:null},usage:{input_tokens:ei.event.usage.prompt_tokens||0,cache_creation_input_tokens:ei.event.usage.cache_creation_input_tokens||0,cache_read_input_tokens:ei.event.usage.cache_read_input_tokens||0,output_tokens:ei.event.usage.completion_tokens||0}})):("stop"===el.finish_reason||"end_turn"===el.finish_reason)&&this.textBlockStarted&&(ea.push({type:"content_block_stop",index:eu}),this.textBlockStarted=!1))}';

let changed = false;

if (!text.includes(constructorNew)) {
  const count = text.split(constructorOld).length - 1;
  if (count !== 1) {
    console.error(`expected one streaming constructor match, got ${count}`);
    process.exit(2);
  }
  text = text.replace(constructorOld, constructorNew);
  changed = true;
  console.log("patched streaming adapter state");
} else {
  console.log("streaming adapter state already patched");
}

if (!text.includes(responseStartedNew)) {
  const count = text.split(responseStartedOld).length - 1;
  if (count !== 1) {
    console.error(`expected one streaming reset match, got ${count}`);
    process.exit(2);
  }
  text = text.replace(responseStartedOld, responseStartedNew);
  changed = true;
  console.log("patched streaming adapter reset");
} else {
  console.log("streaming adapter reset already patched");
}

if (text.includes(adapterToolBlock)) {
  console.log("streaming tool-call adapter already patched");
} else if (text.includes(fallbackToolBlock)) {
  text = text.replace(fallbackToolBlock, adapterToolBlock);
  changed = true;
  console.log("upgraded streaming fallback to adapter");
} else {
  let count = text.split(originalToolBlock).length - 1;
  if (count === 1) {
    text = text.replace(originalToolBlock, adapterToolBlock);
    changed = true;
    console.log("patched streaming tool-call adapter");
  } else {
    count = text.split(oldText).length - 1;
    if (count !== 1) {
      console.error(`expected one streaming tool-call adapter match, got ${count}`);
      process.exit(2);
    }
    text = text.replace(oldText, fallbackText);
    console.error("only patched narrow fallback; full adapter block was not found");
    process.exit(2);
  }
}
...
const toolProfile = 'MoonshotToolContentArrayPlugin.ID_PATTERNS=["kimi-k2.5","kimi-k2.6","kimi-k2p6","kimi-k2p6-turbo"]';
const baseThinking = 'MoonshotThinkingTemperaturePlugin.ID_PATTERNS=["kimi-k2.5","kimi-k2.6"]';
const baseTool = 'MoonshotToolContentArrayPlugin.ID_PATTERNS=["kimi-k2.5","kimi-k2.6"]';
const baseReasoning = 'MoonshotReasoningPlaceholderPlugin.ID_PATTERNS=["kimi-k2.5","kimi-k2.6"]';

text = text.replace(/MoonshotThinkingTemperaturePlugin\.ID_PATTERNS=\[[^\]]+\]/, baseThinking);
text = text.replace(/MoonshotToolContentArrayPlugin\.ID_PATTERNS=\[[^\]]+\]/, toolProfile);
text = text.replace(/MoonshotReasoningPlaceholderPlugin\.ID_PATTERNS=\[[^\]]+\]/, baseReasoning);
fs.writeFileSync(file, text);
console.log("patched Kimi K2.6 custom profile (tool-content only, no thinking)");
NODE
